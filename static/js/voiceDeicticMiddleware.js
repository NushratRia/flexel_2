...
(function (global) {
    ...
    function maybeHandleLocalDeictic(str){
        ...
        //merge this
        if ((has('merge') || has('combine') || has('join')) && anyDeictic) {
        const ok = (global.DeicticRun && global.DeicticRun({ action:'merge', range:'this' })) || false;
        if (ok) { showOpToast('Merged', getTwoHandTargets()); }
        return ok;
        }
    ...
    function parseLocal(raw){
        ...
        if (/\b(merge|combine|join|collide)\b/i.test(s)) {
        if (/\brows?\b/i.test(s))  return { action:'merge', range:'this', confidence:0.93 };
        if (/\b(columns?|cols?)\b/i.test(s)) return { action:'merge', range:'this', confidence:0.93 };
        if (/\bcells?\b/i.test(s)) return { action:'merge', range:'this', confidence:0.93 };
        ...
        // NOTE: bracket fix — keep the rest of merge handling INSIDE this block
        // so later function declarations stay inside a valid scope.
        // (No logic changes; just fixes the broken brace structure.)

        // Existing explicit A1 / ranges still supported:
        const spanCells2 = extractCellSpan(s); if (spanCells2) return { action:'merge_cells', range: spanCells2, confidence:0.96 };
        const cols2 = extractColRange(s);      if (cols2)     return { action:'merge_cols',  range: cols2,      confidence:0.95 };
        const rows2 = extractRowRangeOrSingle(s); if (rows2)  return { action:'merge_rows',  range: rows2,      confidence:0.95 };
        const a12 = extractA1(s);              if (a12)       return { action:'merge_cells', range: a12,        confidence:0.90 };

        // Excel-style column span "A:B" (no row numbers)
        function extractExcelColSpan(s){
        // Match "A:B" (no row digits)
        const m = s.match(/\b([a-z]{1,3})\s*:\s*([a-z]{1,3})\b/i);
        if (!m) return null;
        // If it's actually A2:B2, ignore (cells range handled elsewhere)
        if (/\b[a-z]{1,3}\d+\s*:\s*[a-z]{1,3}\d+\b/i.test(s)) return null;
        return `${m[1].toUpperCase()}:${m[2].toUpperCase()}`; // keep as "A:B"
        }

        function extractExcelRowSpan(s){
        // Match "3:4" (rows)
        const m = s.match(/\b(\d+)\s*:\s*(\d+)\b/);
        if (!m) return null;
        const a = parseInt(m[1], 10), b = parseInt(m[2], 10);
        if (!a || !b) return null;
        return `${Math.min(a,b)}:${Math.max(a,b)}`; // keep as "3:4"
        }

        // Deictic default:
        return { action:'merge_cells', range:'this', confidence:0.90 };
        }

        if (/\b(merge|combine|join|collide)\b/i.test(s)) {
        const spanCells = extractCellSpan(s);     // e.g., "A2:B2"
        if (spanCells) return { action:"merge", range: spanCells, confidence:0.96 };

        const a1 = extractA1(s);                 // e.g., "A2"
        if (a1) return { action:"merge", range: a1, confidence:0.92 };

        const colSpan = extractExcelColSpan(s);  // e.g., "A:B"
        if (colSpan) return { action:"merge", range: colSpan, confidence:0.95 };

        const rowSpan = extractExcelRowSpan(s);  // e.g., "3:4"
        if (rowSpan) return { action:"merge", range: rowSpan, confidence:0.95 };

        // deictic ("merge these/this")
        if (/\b(these|this|here|there|hear|hair)\b/i.test(s) || /^merge\s*$/i.test(s.trim())) {
...
//         // Enhanced to support "merge these", "merge rows", "merge columns" for two-hand deictic selection
//         if (/\b(merge|combine|join|collide)\b/i.test(s)) {
//         // Explicit rows/cols/cells keywords set action:
//         if (/\brows?\b/i.test(s))  return { action:'merge_rows', range:'this', confidence:0.93 };
//         if (/\b(columns?|cols?)\b/i.test(s)) return { action:'merge_cols', range:'this', confidence:0.93 };
//         if (/\bcells?\b/i.test(s)) return { action:'merge_cells', range:'this', confidence:0.93 };

//         // Existing explicit A1 / ranges still supported:
//         const spanCells = extractCellSpan(s); if (spanCells) return { action:'merge_cells', range: spanCells, confidence:0.96 };
//         const cols = extractColRange(s);      if (cols)     return { action:'merge_cols',  range: cols,      confidence:0.95 };
//         const rows = extractRowRangeOrSingle(s); if (rows)  return { action:'merge_rows',  range: rows,      confidence:0.95 };
//         const a1 = extractA1(s);              if (a1)       return { action:'merge_cells', range: a1,        confidence:0.90 };
